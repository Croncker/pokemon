...
let pokemon = fetch("https://pokeapi.co/api/v2/pokemon/" + randomNumber)
    ...
    let tvShow = fetch("https://api.tvmaze.com/search/shows?q=%20the%20flash")
    .then(function(response){
        return response.json();
    })
    .then(function(realData){
        tvShowTitel.innerHTML = realData[0].show.name;
        tvShowTekst.innerHTML = realData[0].show.summary;
    })
    ...
    const data = {
        ...
        datasets:[
            {
                label: "5 most powerful Pokemons",
...
function getPokemon() {
    ...
    const labels = ['normal','fighthing','flying','poison','ground','rock',
...
function getPokemon2() {
    ...
    for (i = 0; i < 10; i++) {
        ...
        let pokemon2 = fetch("https://pokeapi.co/api/v2/pokemon/" + random2)
            .then(function (response) {
                return response.json();
            })
            .then(function (pokemon2) {
                switch (pokemon2.types[0].type.name) {
                        case 'normal':
                            normal = normal + 1;
                            break;                    
                        case 'fighthing':
                ...
                dataPokemon2.datasets[0].data = [normal, fighting, flying, poison, ground, rock, 
                    bug, ghost, steel, fire, water, grass, electric, psychic,
                    ice, dragon, dark, fairy, unknown, shadow];
                        graph2.update();
                })
